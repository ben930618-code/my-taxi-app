...
let currentDutyPlate = 'ABC-1234'; // 當前值班幹部 (預設 ABC-1234)
let clientRegistry = [
...
app.get('/admin', (req, res) => {
    const htmlLines = [
        ...
        '<style>body{font-family:sans-serif; background:#f4f6f9; padding:15px; margin:0;} .card{background:white; padding:20px; border-radius:10px; box-shadow:0 2px 8px rgba(0,0,0,0.06); margin-bottom:20px;} h2,h3,h4{margin-top:0; color:#333; border-bottom:2px solid #007bff; padding-bottom:6px;} table{width:100%; border-collapse:collapse; margin-top:10px; font-size:14px;} th,td{border:1px solid #ddd; padding:10px; text-align:left;} th{background:#e9ecef;} .badge{padding:4px 8px; border-radius:4px; font-size:12px; font-weight:bold; color:white;} .bg-gray{background:#6c757d;} .bg-blue{background:#007bff;} .bg-orange{background:#fd7e14;} .bg-green{background:#28a745;} .bg-purple{background:#6f42c1;} .bg-red{background:#dc3545;} .inp{width:100%; padding:8px; box-sizing:border-box; margin-top:4px;} .duty-highlight{background:#fff5f5; border:2px solid #dc3545; padding:15px; border-radius:8px; margin-bottom:15px;}</style></head><body>',
        
        '',
        '<div class="card duty-highlight">',
        '   <h2 style="color:#dc3545; border-bottom:2px solid #dc3545;">📅 系統值班幹部手動排班/切換區 (30天6小時制)</h2>',
        '   <p style="font-size:15px; color:#555; margin:5px 0 12px 0;">目前系統設定的【當值調度幹部】車牌為：<span id="currentDutyDisplay" style="font-size:18px; color:red; font-weight:bold; background:#fff; padding:2px 8px; border:1px solid red; border-radius:4px;">載入中...</span></p>',
        '   <label><b>🔄 指派新值班幹部：</b></label>',
        '   <select id="dutyCadreSelect" class="inp" style="width:auto; min-width:200px; display:inline-block; margin-right:10px; font-weight:bold; color:purple;"></select>',
        '   <button onclick="changeDutyCadre()" style="padding:8px 16px; background:#dc3545; color:white; border:none; font-weight:bold; border-radius:4px; cursor:pointer;">執行換班確認</button>',
        '</div>',
        ...
        '<div class="card"><h3>🚖 線上司機即時動態</h3><div id="driverStatusDiv">等待資料載入...</div></div>',
        ...
        'function changeDutyCadre() { const selectedPlate = document.getElementById("dutyCadreSelect").value; if(selectedPlate) { socket.emit("admin_set_duty", { plate: selectedPlate }); alert("值班幹部已成功變更為: " + selectedPlate); } }',
        ...
        '    const selectBox = document.getElementById("dutyCadreSelect"); const currentVal = selectBox.value; selectBox.innerHTML = ""; for(let k in data.driverRegistry) { if(data.driverRegistry[k].role === "cadre") { let opt = document.createElement("option"); opt.value = k; opt.innerText = `${k} (${data.driverRegistry[k].name})`; if(k === data.currentDutyPlate) opt.selected = true; selectBox.appendChild(opt); } }',
