...
// ⚙️ 核心邏輯：啟動訂單自動計時器（2分鐘轉一般，再2分鐘轉外部）
function startOrderTimers(orderId) {
    setTimeout(() => {
        ...
        if (ord && ord.status === "幹部審核優先中") {
            ...
            // 下放一般司機後，再過 2 分鐘如果還是無人承接，直接拋出外部
            setTimeout(() => {
...
app.get('/admin', (req, res) => {
    const htmlLines = [
        ...
        '<div class="card"><h2>🚨 管理者發送新派單 (預設進入幹部優先池)</h2>',
        '<div style="margin-bottom:12px;"><label><b>1. 訂單類型:</b></label><select id="orderType" class="inp" onchange="toggleTimeInput()"><option value="即時單">⚡ 即時派單 (立刻用車)</option><option value="預約單">📅 預約派單 (指定時間)</option></select></div>',
        '<div id="bookingTimeDiv" style="display:none; margin-bottom:12px; background:#e9ecef; padding:10px; border-radius:5px;"><label>預約用車時間:</label><input type="datetime-local" id="bookingTime" class="inp"></div>',
        '<div style="margin-bottom:12px;"><label><b>2. 上車地址：</b></label><input type="text" id="addr" value="桃園市桃園區中正路1號" class="inp"></div>',
        ...
        '<button onclick="sendOrder()" style="width:100%; padding:12px; background:blue; color:white; border:none; font-size:16px; font-weight:bold; border-radius:5px; cursor:pointer;">建立訂單 (送入車隊池)</button></div>',
        '<div class="card"><h3>🚖 線上司機即時動態 (幹部與值班切換)</h3><div id="driverStatusDiv">等待資料載入...</div></div>',
        '<div class="card"><h3>📋 歷史與當前派單總表</h3><div style="overflow-x:auto;"><table><thead><tr><th>時間/類型</th><th>乘客上車點</th><th>擔當司機</th><th>當前狀態</th><th>結帳回報</th></tr></thead><tbody id="orderTableBody"></tbody></table></div></div>',
        ...
        'function sendOrder() { const type = document.getElementById("orderType").value; let bTime = "無 (即時單)"; if(type === "預約單") { const rawTime = document.getElementById("bookingTime").value; if(!rawTime) { alert("請選擇預約時間！"); return; } bTime = rawTime.replace("T", " "); } fetch("/api/dispatch", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({ targetAddress: document.getElementById("addr").value, targetLat: parseFloat(document.getElementById("lat").value), targetLng: parseFloat(document.getElementById("lng").value), orderType: type, bookingTime: bTime }) }); }',
        'function setDuty(plate) { socket.emit("admin_set_duty", { plate }); }',
        'function addDriver() { const plate = document.getElementById("newPlate").value.trim().toUpperCase(); const name = document.getElementById("newName").value.trim(); const phone = document.getElementById("newPhone").value.trim(); const role = document.getElementById("newRole").value; if(!plate || !name || !phone) { alert("請填齊資料"); return; } socket.emit("admin_add_driver", { plate, name, phone, role }); document.getElementById("newPlate").value=""; document.getElementById("newName").value=""; document.getElementById("newPhone").value=""; }',
        ...
        '    const drDiv = document.getElementById("driverStatusDiv"); if(data.drivers.length === 0) { drDiv.innerHTML = "<span style=\'color:gray;\'>目前沒有司機上線</span>"; } else { let drHtml = ""; data.drivers.forEach(d => { let isCadre = data.driverRegistry[d.id]?.role === "cadre"; let roleTag = isCadre ? "<b style=\'color:purple;\'>[車隊幹部]</b>" : "[一般司機]"; let dutyBtn = ""; if(isCadre) { if(data.currentDutyPlate === d.id) { dutyBtn = " <span style=\'background:green; color:white; padding:2px 6px; border-radius:3px; font-size:12px;\'>[🟢 當值中]</span>"; } else { dutyBtn = " <button onclick=\\"setDuty(\'" + d.id + "\')\\" style=\'font-size:11px; cursor:pointer;\'>設為今日值班</button>"; } } let statusBadge = d.isBusy ? "<span style=\'color:red;\'>[🔴 任務中]</span>" : "<span style=\'color:green;\'>[🟢 空車]</span>"; drHtml += "<div style=\'padding:6px 0; border-bottom:1px dashed #eee;\'>🚗 <b>" + d.name + " (" + d.id + ")</b> " + roleTag + " - " + statusBadge + dutyBtn + "</div>"; }); drDiv.innerHTML = drHtml; }',
        '    const oBody = document.getElementById("orderTableBody"); if(data.orders.length === 0) { oBody.innerHTML = "<tr><td colspan=\'5\' style=\'text-align:center; color:gray;\'>暫無派單紀錄</td></tr>"; } else { let oHtml = ""; data.orders.slice().reverse().forEach(o => { let statusStr = `<span class="badge bg-gray">${o.status}</span>`; if(o.status === "幹部審核優先中") statusStr = "<span class=\'badge bg-purple\'>幹部優先(剩餘2分)</span>"; if(o.status === "開放一般司機搶單") statusStr = "<span class=\'badge bg-blue\'>一般司機搶單中</span>"; if(o.status === "前往迎客") statusStr = `<span class="badge bg-blue">前往迎客 (${o.eta || "?"}分)</span>`; if(o.status === "旅客已上車") statusStr = "<span class=\'badge bg-orange\'>旅客已上車</span>"; if(o.status === "行程已完成") statusStr = "<span class=\'badge bg-green\'>行程已完成</span>"; if(o.status.includes("2分逾時")) statusStr = `<span class="badge bg-red">${o.status}</span>`; oHtml += "<tr><td>" + o.bookingTime + "<br><small>(" + o.orderType + ")</small></td><td>" + o.targetAddress + "</td><td>" + (o.driverName ? o.driverName : "-") + "</td><td>" + statusStr + "</td><td>" + (o.paymentReport || "-") + "</td></tr>"; }); oBody.innerHTML = oHtml; }',
...
app.get('/driver', (req, res) => {
    const htmlLines = [
        ...
        '<div id="cadreDashboard" class="box" style="display:none; border:2px solid #6f42c1; background:#fdfbfe;"><h3 style="margin-top:0; color:#6f42c1;">👑 幹部調度查閱大盤 (前2分限幹部)</h3><div id="cadreOrderList" style="font-size:14px; color:#333;">讀取中...</div></div>',
        '<div id="pop" style="display:none; background:#fff3cd; border:2px solid #ffecb5; padding:20px; border-radius:10px; margin-bottom:15px; text-align:left;"><h3 style="color:#856404; margin-top:0; text-align:center;">🚨 收到任務搶單廣播！</h3><p><b>單類：</b><span id="popOrderType" style="color:blue; font-weight:bold;"></span></p><p><b>乘客上車點：</b><span id="addrText" style="font-weight:bold;"></span></p><button id="acceptBtn" class="btn btn-green" style="font-size:18px;">立刻按此接單 (搶)</button></div>',
        '<div id="currentMissionSection" class="box" style="display:none; border:2px solid #007bff; background:#f8f9fa;"><h3 style="margin-top:0; color:#007bff; text-align:center;">📍 當前執行中任務</h3><p><b>上車點：</b><span id="missionAddr" style="font-weight:bold;"></span></p><p id="missionEtaRow" style="color:red; font-weight:bold;"></p><button onclick="clickNav()" class="btn btn-blue">🧭 開啟 Google Map 導航</button><div id="step1_board" style="margin-top:10px;"><button onclick="reportBoarded()" class="btn btn-orange" style="margin-bottom:8px;">🙋‍♂️ 客人已上車</button><button onclick="cancelMyOrder()" class="btn btn-red" style="padding:8px; font-size:14px;">🚨 我有急事/取消接單(退單)</button></div><div id="step2_complete" style="margin-top:10px; display:none;"><button onclick="showCompleteModal()" class="btn btn-red">🏁 客人已下車 (結帳)</button></div></div>',
        '<div id="scheduleSection" class="box" style="display:none;"><h3 style="margin-top:0; color:#333; border-bottom:2px solid #6f42c1;">📅 全區可接單清單 (包含未接預約單)</h3><div id="scheduleList" style="font-size:14px; color:#555;">暫無可接訂單</div></div>',
        ...
        'socket.on("login_success", (data) => { isOnline = true; myRole = data.role; myPlate = document.getElementById("plateNum").value.trim().toUpperCase(); document.getElementById("toggleBtn").innerText = "關閉下班 (停止定位)"; document.getElementById("toggleBtn").className = "btn btn-red"; document.getElementById("status").innerText = "🟢 線上空車候客中..."; document.getElementById("status").style.color = "green"; document.getElementById("plateNum").disabled = true; document.getElementById("phoneNum").disabled = true; let roleStr = (myRole === "cadre") ? "【👑 車隊幹部】" : "【🚖 一般司機】"; if(data.isDuty) { roleStr += " ⭐ 今日值班幹部"; } document.getElementById("roleTag").innerText = roleStr; if(myRole === "cadre") { document.getElementById("cadreDashboard").style.display = "block"; } document.getElementById("scheduleSection").style.display = "block"; socket.emit("get_available_orders"); });',
        ...
        'function resetToOfflineInfo() { isOnline = false; document.getElementById("toggleBtn").innerText = "驗證並開啟上班"; document.getElementById("toggleBtn").className = "btn btn-green"; document.getElementById("status").innerText = "🔴 目前下班中 (未定位)"; document.getElementById("roleTag").innerText = ""; document.getElementById("cadreDashboard").style.display = "none"; document.getElementById("scheduleSection").style.display = "none"; document.getElementById("currentMissionSection").style.display = "none"; if (watchId) navigator.geolocation.clearWatch(watchId); }',
        ...
        'socket.on("sync_orders_to_driver", (data) => { if(!isOnline) return; if(myRole === "cadre") { let html = ""; data.orders.forEach(o => { let opBtns = ""; if(o.status === "幹部審核優先中" && data.currentDutyPlate === myPlate) { opBtns = ` <button class="btn-small btn-blue" onclick="cadreReleaseDirectly(\'${o.orderId}\')">提前放給一般司機</button>`; } html += `<div style="border-bottom:1px solid #eee; padding:6px 0;">⏰ <b>${o.bookingTime}</b> (${o.orderType})<br>📍 ${o.targetAddress}<br>狀態: <span style="color:purple;font-weight:bold;">${o.status}</span>${opBtns}</div>`; }); document.getElementById("cadreOrderList").innerHTML = html || "當前無任何訂單"; }',
        '    const listDiv = document.getElementById("scheduleList"); let filterOrders = data.orders.filter(o => { if(["行程已完成", "前往迎客", "旅客已上車"].includes(o.status)) return false; if(myRole === "driver" && o.status === "幹部審核優先中") return false; return true; }); if(filterOrders.length === 0) { listDiv.innerHTML = "暫無可接訂單"; return; } let h = ""; filterOrders.forEach(o => { let typeTag = o.orderType === "預約單" ? `<b style="color:purple;">[📅預約]</b>` : `<b>[⚡即時]</b>`; let takeBtn = `<button class="btn-small btn-green" onclick="clickTakeOrder(\'${o.orderId}\')">手動接此單</button>`; h += `<div style="background:#f1f3f5; padding:8px; border-left:4px solid #007bff; margin-top:5px;">${typeTag} <b>[${o.bookingTime}]</b> ${o.targetAddress}<br>狀態: ${o.status} ${takeBtn}</div>`; }); listDiv.innerHTML = h; });',
        'function cadreReleaseDirectly(orderId) { socket.emit("cadre_release_force", { orderId, plateNumber: myPlate }); }',
        'function clickTakeOrder(orderId) { socket.emit("accept_order", { orderId, plateNumber: myPlate, lat: currentLat, lng: currentLng }); }',
        'socket.on("new_order_request", (data) => { if(currentActiveMission) return; if(data.forRole === "cadre" && myRole !== "cadre") return; document.getElementById("popOrderType").innerText = data.orderType; document.getElementById("addrText").innerText = data.targetAddress; document.getElementById("pop").style.display = "block"; document.getElementById("acceptBtn").onclick = function() { socket.emit("accept_order", { orderId: data.orderId, plateNumber: myPlate, lat: currentLat, lng: currentLng }); }; });',
        'socket.on("accept_result", (data) => { document.getElementById("pop").style.display = "none"; if(data.success) { currentActiveMission = data.order; document.getElementById("status").innerText = "🚖 任務執行中 (前往上車點)"; document.getElementById("status").style.color = "blue"; document.getElementById("missionAddr").innerText = data.order.targetAddress; document.getElementById("missionEtaRow").innerText = "⏳ 預計 " + (data.eta || "?") + " 分鐘後抵達上車地址"; document.getElementById("currentMissionSection").style.display = "block"; document.getElementById("step1_board").style.display = "block"; document.getElementById("step2_complete").style.display = "none"; } else { alert(data.message); } socket.emit("get_available_orders"); });',
        'function cancelMyOrder() { if(!currentActiveMission) return; if(confirm("確定要取消這筆接單任務嗎？(此單會重新釋放入車隊池)")) { socket.emit("driver_cancel_order", { orderId: currentActiveMission.orderId, plateNumber: myPlate }); document.getElementById("currentMissionSection").style.display = "none"; currentActiveMission = null; document.getElementById("status").innerText = "🟢 線上空車候客中..."; document.getElementById("status").style.color = "green"; } }',
        ...
        'function reportBoarded() { socket.emit("driver_report_status", { orderId: currentActiveMission.orderId, status: "旅客已上車", plateNumber: myPlate }); document.getElementById("status").innerText = "🚖 旅客運送中..."; document.getElementById("step1_board").style.display = "none"; document.getElementById("step2_complete").style.display = "block"; }',
...
io.on('connection', (socket) => {
    ...
    socket.on('driver_cancel_order', (data) => {
        ...
        if(ord && ord.driverId === pNum) {
            if(driverInfo) driverInfo.isBusy = false; // 解鎖司機

            const timeElapsed = Date.now() - ord.createdAt;
            
            // 判斷退單時，這張單的生存時間是否還在 2 分鐘內
