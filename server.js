...
// 💡 記憶體優化優化機制：主動限制 V8 引擎的最大記憶體使用，避免衝過免費版 512MB 臨界點
if (global.gc) {
...
const io = socketIo(server, {
    ...
    pingTimeout: 30000, // 縮短超時時間，快點釋放斷線司機的記憶體
    pingInterval: 15000
});
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (!dutySchedule30Days[dateStr]) {
            dutySchedule30Days[dateStr] = {
                "00_06": 'ABC-1234', "06_12": 'ABC-1234',
                "12_18": 'ABC-1234', "18_24": 'ABC-1234'
            }; 
        }
    }
}
...
function getCurrentDutyPlate() {
    ...
    return dutySchedule30Days[dateStr] ? dutySchedule30Days[dateStr][slot] : 'ABC-1234';
}
...
// ─── ⏰ 預約單自動釋放機制 (拉長檢查間隔至 30 秒，減少 CPU 與記憶體負擔) ───
setInterval(() => {
    const now = Date.now();
    let updated = false;
    ...
    // 💡 記憶體防爆：如果歷史訂單累積超過 150 筆，自動切掉最舊的，避免撐爆陣列
    if (globalOrders.length > 150) {
        globalOrders = globalOrders.slice(-100);
    }
    if (creditLedger.length > 200) {
        creditLedger = creditLedger.slice(-150);
    }
}, 30000);
...
app.get('/admin', (req, res) => {
    const htmlLines = [
        ...
        '<h2>🚕 豪車隊中央大盤調度中心</h2>',
        '<button class="setting-btn" onclick="toggleSettingPanel()">🛠️ 點此「開啟 / 關閉」系統核心設定專區</button>',
        '<div id="settingPanel" class="card" style="display:none; border: 3px solid #495057; background:#f8f9fa;">',
        '   <h3 style="color:#28a745; border-bottom:2px solid #28a745; margin-top:15px;">🗓️ 未來 30 天值班幹部手動排班</h3>',
        ...
        '        let item = document.createElement("div"); item.style = "background:white; padding:10px; border:1px solid #ccc; border-radius:6px; margin-bottom:5px;";',
        ...
        '        slots.forEach(s => {',
        '            let row = document.createElement("div"); row.style="margin-top:4px; font-size:12px;"; row.innerText = s.n + "：";',
        '            let sel = document.createElement("select"); sel.style="width:100%; padding:2px;"; sel.onchange = function() { updateSlotDuty(dateStr, s.k, this); };',
        ...
        '    });',
        '    const drDiv = document.getElementById("driverStatusDiv"); if(data.drivers.length === 0) { drDiv.innerHTML = "<span style=\'color:gray;\'>目前沒有司機上線</span>"; } else { let drHtml = ""; data.drivers.forEach(d => { let isCadre = data.driverRegistry[d.id]?.role === "cadre"; let roleTag = isCadre ? "<b style=\'color:purple;\'>[幹部]</b>" : "[司機]"; let dutyTag = (data.currentDutyPlate === d.id) ? " <span style=\'background:green; color:white; padding:1px 4px; border-radius:3px; font-size:11px;\'>[值班中]</span>" : ""; let statusBadge = d.isBusy ? "<span style=\'color:red;\'>[🔴任務中]</span>" : "<span style=\'color:green;\'>[🟢空車]</span>"; drHtml += "<div>🚗 <b>" + d.name + " (" + d.id + ")</b> " + roleTag + " " + statusBadge + dutyTag + "</div>"; }); drDiv.innerHTML = drHtml; }',
        '    const oBody = document.getElementById("orderTableBody"); if(data.orders.length === 0) { oBody.innerHTML = "<tr><td colspan=\'6\' style=\'text-align:center; color:gray;\'>暫無派單紀錄</td></tr>"; } else { let oHtml = ""; data.orders.slice().reverse().forEach(o => { let statusStr = `<span class="badge bg-gray">${o.status}</span>`; if(o.status === "等") statusStr = "<span class=\'badge bg-purple\'>⏳ 幹部抉擇中</span>"; if(o.status === "讓") statusStr = "<span class=\'badge bg-purple\'>👑 幹部優先</span>"; if(o.status === "丟") statusStr = "<span class=\'badge bg-blue\'>👥 全區搶單</span>"; if(o.status === "前往迎客") statusStr = `<span class="badge bg-blue">前往迎客 (${o.eta || "?"}分)</span>`; if(o.status === "旅客已上車") statusStr = "<span class=\'badge bg-orange\'>旅客已上車</span>"; if(o.status === "行程已完成") statusStr = "<span class=\'badge bg-green\'>行程已完成</span>"; if(o.status === "顧客已取消") statusStr = "<span class=\'badge bg-red\'>❌ 顧客已取消</span>"; if(o.status === "已預約排定") statusStr = "<span class=\'badge bg-green\'>📅 預約已接單</span>";',
        '                    let reports = []; if(o.reportedHighSpeed) reports.push("<b style=\'color:purple;\'>⚠️ 已上高速</b>"); if(o.reportedWaitingTooLong) reports.push("<b style=\'color:darkorange;\'>⚠️ 沒看到客人</b>"); if(reports.length===0) reports.push("<span style=\'color:gray;\'>正常</span>"); if(o.status === "顧客已取消") reports = ["-"];',
        '                    let timeLabel = o.orderType === "預約單" ? `<b>[預約]</b><br>${o.bookingTime}` : `<b>[即時]</b><br>${o.bookingTime}`;',
        '                    let cancelBtn = (["行程已完成", "顧客已取消"].includes(o.status)) ? "-" : `<button class="btn-cancel" onclick="adminCancelOrder(\'${o.orderId}\')">❌ 取消</button>`;',
        ...
        '    let totalSum = 0; let lHtml = ""; data.ledger.forEach(l => { totalSum += l.amount; lHtml += `<tr><td>${l.time}</td><td>${l.driverId}</td><td><b>${l.clientName}</b></td><td>$${l.amount}</td></tr>`; }); document.getElementById("ledgerTableBody").innerHTML = lHtml; document.getElementById("ledgerTotal").innerText = totalSum;',
...
app.get('/driver', (req, res) => {
    const htmlLines = [
        ...
        '        <button onclick="reportTooLong()" class="btn-small btn-purple">⏳ 沒看到客人</button>',
        '    </div>',
        ...
        '    <h3 style="margin-top:0; color:brown;">📅 我的預約行程表</h3>',
        ...
        'let isOnline = false; let watchId = null; let currentLat = 24.9936; let currentLng = 121.3130; let currentActiveMission = null; let myRole = "driver"; let myPlate = "";',
        ...
        '   if(!isOnline) { socket.emit("driver_location_update", { plateNumber: pNum, phoneNumber: pwd, lat: currentLat, lng: currentLng }); }',
        '   else { socket.emit("driver_offline", { plateNumber: myPlate }); resetToOfflineInfo(); }',
        '}',
        ...
        '    if(navigator.geolocation) { watchId = navigator.geolocation.watchPosition(pos => { currentLat = pos.coords.latitude; currentLng = pos.coords.longitude; if(isOnline) { socket.emit("driver_location_update", { plateNumber: myPlate, phoneNumber: document.getElementById("phoneNum").value.trim(), lat: currentLat, lng: currentLng }); } }, null, {enableHighAccuracy:false}); }', // 💡 改為 false 降級精確度，省電且防爆
        '    socket.emit("get_available_orders");',
        '});',
        'function resetToOfflineInfo() { isOnline = false; document.getElementById("toggleBtn").innerText = "確認登入上班"; document.getElementById("toggleBtn").className = "btn btn-green"; document.getElementById("status").innerText = "🔴 目前離線狀態"; document.getElementById("status").style.color = "gray"; document.getElementById("roleTag").innerText = ""; document.getElementById("dutyCadreSection").style.display = "none"; document.getElementById("cadreWeeklyReservationsBox").style.display = "none"; document.getElementById("scheduleSection").style.display = "none"; document.getElementById("myReservationsSection").style.display = "none"; document.getElementById("currentMissionSection").style.display = "none"; document.getElementById("plateNum").disabled = false; document.getElementById("phoneNum").disabled = false; if(watchId) navigator.geolocation.clearWatch(watchId); }',
        'function syncDutyUI(dutyPlate) { let isDuty = (dutyPlate === myPlate); let roleStr = (myRole === "cadre") ? "【👑 車隊幹部】" : "【🚖 一般司機】"; if(isDuty) { roleStr += " ⭐ 值班中"; document.getElementById("dutyCadreSection").style.display = "block"; } else { document.getElementById("dutyCadreSection").style.display = "none"; } document.getElementById("roleTag").innerText = roleStr; }',
        ...
        '    const select = document.getElementById("clientSelect"); select.innerHTML = ""; (data.clientRegistry || []).forEach(c => { let opt = document.createElement("option"); opt.value = c.name; opt.innerText = c.name; select.appendChild(opt); });',
        '    if(!isOnline) return;',
        '    if(myRole === "cadre") {',
        '        let weeklyOrders = (data.orders || []).filter(o => o.orderType === "預約單" && o.status === "等");',
        '        let wh = ""; weeklyOrders.forEach(o => { wh += `<div style="background:white; border:1px solid #6f42c1; padding:8px; margin-top:5px; border-radius:4px;">📅 <b>[${o.serviceType}]</b> ${o.targetAddress}<br><small>預約: ${o.bookingTime}</small><br><button class="btn-small btn-purple" onclick="clickTakeOrder(\'${o.orderId}\')">👑 接此預約</button></div>`; });',
        ...
        '            document.getElementById("currentMissionSection").style.display = "none"; currentActiveMission = null;',
        '            document.getElementById("status").innerText = "🟢 線上候客中..."; document.getElementById("status").style.color = "green";',
        '        }',
        '    }',
        '    if(isDuty) { let html = ""; data.orders.forEach(o => { if(o.status === "等" && o.orderType !== "預約單") { html += `<div style="border-bottom:1px dashed #ccc; padding:8px 0;">📍 <b>[${o.serviceType}]</b> ${o.targetAddress}<br><button class="btn-small btn-green" onclick="dutyAction(\'${o.orderId}\', \'接\')">🟢 我接了</button> <button class="btn-small btn-blue" onclick="dutyAction(\'${o.orderId}\', \'丟\')">🔵 丟給司機</button> <button class="btn-small btn-purple" onclick="dutyAction(\'${o.orderId}\', \'讓\')">🟣 幹部優先</button></div>`; } }); document.getElementById("dutyOrderList").innerHTML = html || "暫無即時案件需處理"; }',
        '    const listDiv = document.getElementById("scheduleList"); let filterOrders = data.orders.filter(o => { if(["行程已完成", "前往迎客", "旅客已上車", "顧客已取消"].includes(o.status)) return false; if(o.status === "等") return false; if(myRole === "driver" && o.status === "讓") return false; return true; }); if(filterOrders.length === 0) { listDiv.innerHTML = "暫無開放搶單的項目"; } else { let h = ""; filterOrders.forEach(o => { let typeTag = o.orderType === "預約單" ? "<span>[📅開放預約]</span>" : "<span>[⚡即時]</span>"; h += `<div style="background:#f1f3f5; padding:8px; margin-top:5px;">${typeTag} <b>[${o.serviceType}]</b> ${o.targetAddress}<br><button class="btn-small btn-green" onclick="clickTakeOrder(\'${o.orderId}\')">立刻搶單</button></div>`; }); listDiv.innerHTML = h; }',
        '    const resDiv = document.getElementById("myReservationsList"); let myResOrders = data.orders.filter(o => o.driverId === myPlate && o.orderType === "預約單"); if(myResOrders.length === 0) { resDiv.innerHTML = "暫無排定預約行程"; } else { let rh = ""; myResOrders.forEach(o => { let actionBtn = o.status === "已預約排定" ? `<button class="btn-small btn-blue" onclick="startReservationMission(\'${o.orderId}\')">🚀 現在出發迎客</button>` : `<span>[狀態: ${o.status}]</span>`; rh += `<div style="background:#fff; border:1px solid brown; padding:8px; margin-top:5px;"><b>[${o.serviceType}]</b> ${o.targetAddress}<br>${actionBtn}</div>`; }); resDiv.innerHTML = rh; }',
        '});',
...
app.post('/api/dispatch', (req, res) => {
    ...
    let newOrder = { 
        ...
        driverNote: note, status: "等", driverId: null, driverName: null, paymentReport: null, eta: null,
        reportedHighSpeed: false, reportedWaitingTooLong: false, autoReleasedToDrivers: false, createdAt: Date.now() 
    };
...
io.on('connection', (socket) => {
    ...
    socket.on('admin_cancel_order', (data) => {
        ...
        if(ord) {
            ord.status = "顧客已取消";
            if(ord.driverId && activeDrivers[ord.driverId]) { activeDrivers[ord.driverId].isBusy = false; }
            broadcastAdminData(); broadcastToDrivers();
        }
    });
    ...
    socket.on('driver_location_update', (data) => {
        ...
        if (!registeredDriver || registeredDriver.phone !== data.phoneNumber) {
            socket.emit('login_failed', { message: "❌ 登入失敗！" });
            return;
        }
    ...
    socket.on('duty_cadre_decision', (data) => {
        ...
        if (data.action === '接') {
            ...
            ord.status = "丟";
            Object.values(activeDrivers).forEach(driver => {
                if (!driver.isBusy) { io.to(driver.socketId).emit('new_order_request', { ...ord, targetScope: 'all' }); }
            });
        } else if (data.action === '讓') {
            ord.status = "讓";
            Object.values(activeDrivers).forEach(driver => {
                let regInfo = driverRegistry[driver.id];
                if (regInfo && regInfo.role === 'cadre' && !driver.isBusy) { io.to(driver.socketId).emit('new_order_request', { ...ord, targetScope: 'cadre_only' }); }
            });
        }
        broadcastAdminData(); broadcastToDrivers();
    });
    ...
    socket.on('accept_order', (data) => {
        ...
        if (ord && canTake) {
            ...
            if(regInfo.role === 'driver' && ord.status === "讓") return;

            const realDist = getDistance(ord.targetLat, ord.targetLng, data.lat, data.lng);
            const durationEta = calculateETA(realDist);
            ord.driverId = pNum; ord.driverName = driverInfo ? driverInfo.name : pNum;
    ...
    socket.on('driver_trigger_reservation', (data) => {
        ...
        if(ord && ord.driverId === pNum && ord.status === "已預約排定") {
            ...
            ord.status = "前往迎客"; ord.eta = calculateETA(realDist);
            if(driverInfo) driverInfo.isBusy = true;
            socket.emit('accept_result', { success: true, order: ord, eta: ord.eta, isTriggeredReservation: true });
            broadcastAdminData(); broadcastToDrivers();
        }
    });
    ...
    socket.on('driver_cancel_order', (data) => {
        ...
        if(ord && ord.driverId === pNum) {
            ...
            ord.status = "等"; ord.driverId = null; ord.driverName = null; ord.eta = null;
