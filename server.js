...
// 🗓️ 未來 30 天排班表資料庫 (Key 為 YYYY-MM-DD, Value 為當天車牌)
let dutySchedule30Days = {};

// 初始化未來 30 天的排班 (預設都先給 ABC-1234，管理者可於後台隨時修改)
function init30DaysSchedule() {
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (!dutySchedule30Days[dateStr]) {
            dutySchedule30Days[dateStr] = 'ABC-1234'; 
        }
    }
}
init30DaysSchedule();
...
function getCurrentDutyPlate() {
    ...
    const dateStr = `${yyyy}-${mm}-${dd}`;
    return dutySchedule30Days[dateStr] || 'ABC-1234'; 
}
...
app.get('/admin', (req, res) => {
    const htmlLines = [
        ...
        'function updateDateDuty(dateStr, selectElem) { socket.emit("admin_update_date_duty", { dateStr, plate: selectElem.value }); }',
        ...
        '        let item = document.createElement("div"); item.style = "background:#f8f9fa; padding:8px; border:1px solid #ddd; border-radius:5px;";',
        '        item.innerHTML = `<div><b>📅 ${dateStr}</b></div>`;',
        '        let sel = document.createElement("select"); sel.className = "inp"; sel.style="margin-top:4px; font-size:12px;"; sel.onchange = function() { updateDateDuty(dateStr, this); };',
        '        for(let k in data.driverRegistry) { if(data.driverRegistry[k].role === "cadre") { let opt = document.createElement("option"); opt.value = k; opt.innerText = `${k}(${data.driverRegistry[k].name})`; if(k === data.dutySchedule30Days[dateStr]) opt.selected = true; sel.appendChild(opt); } }',
        '        item.appendChild(sel); board.appendChild(item);',
        '    });',
...
app.get('/driver', (req, res) => {
    const htmlLines = [
        ...
        'function cancelMyOrder() { if(!currentActiveMission) return; if(confirm("確定要取消這筆接單任務嗎？(此單會重新釋回調度池)")) { socket.emit("driver_cancel_order", { orderId: currentActiveMission.orderId, plateNumber: myPlate }); document.getElementById("currentMissionSection").style.none; currentActiveMission = null; document.getElementById("status").innerText = "🟢 線上空車候客中..."; document.getElementById("status").style.color = "green"; } }',
...
io.on('connection', (socket) => {
    ...
    // 📅 核心追加：管理者修改30天內某天特定的值班人員
    socket.on('admin_update_date_duty', (data) => {
        if(data.dateStr && data.plate) {
            dutySchedule30Days[data.dateStr] = data.plate;
